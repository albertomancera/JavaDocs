...
public class IntervaloTiempo {
   ...
   * @param minutos Minutos del intervalo. Debe estar entre 0 y 59, ambos incluidos
   * @param segundos segundos del intervalo. Debe valer entre 0 y 59, ambos incluidos
   * @throws IllegalArgumentException Si minutos o segundos están fuera del rango 0-59, ambos incluidos
   */
  public IntervaloTiempo(int horas, int minutos, int segundos) {
    if ((minutos >= 0) && (minutos <= 59) && (segundos >= 0) && (segundos <= 59)) {
      // No podemos usar el constructor privado por lo que tenemos que hacer aqui
      // lo que se hacía allí
      this.segundos = segundos + minutos * SEGUNDOSPORMINUTO + horas * SEGUNDOSPORHORA;
    } else {
      throw new IllegalArgumentException("Número de minutos o segundos incorrecto");
    }
  }
   ...
   * @return Nuevo IntervaloTiempo con la diferencia
   * @throws IntervaloTiempoException Si el este intervalo es más corto que otro
   */
  public IntervaloTiempo resta(IntervaloTiempo otro) throws IntervaloTiempoException {
    ...
    if (this.segundos >= otro.segundos) {
      ...
      throw new IntervaloTiempoException("No se puede restar un intervalo anterior de otro posterior");
    }
  }
