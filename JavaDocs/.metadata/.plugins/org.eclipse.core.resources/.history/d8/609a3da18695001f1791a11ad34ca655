...
public class Arraybidimensional {
	...
	int sumanotas;
	double media;
	String alumno = entradaTeclado.next();
	
	int [][]notas = new int [alumnos][modulos];
	String [] nombreAlumnos = new String [alumnos];
	String[] materia = {"Matematicas", "Lengua", "Inglés"};
	...
	for(int i=0; i<notas.length; i++) {
		System.out.print(alumno +" " + nombreAlumnos[i] + ":  ");
		sumanotas = 0;
		for(int j=0; j< modulos; j++){
		...
		media= (double) sumanotas /modulos;
